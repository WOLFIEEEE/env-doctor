...
async function runScan(directory, options) {
  const rootDir = (0, import_node_path7.resolve)(directory);
  if (options.verbose) {
    logger.setVerbose(true);
  }
  try {
...
async function runInit(options) {
  ...
  console.log();
  console.log(import_picocolors3.default.bold(import_picocolors3.default.cyan("env-doctor")) + " init");
  console.log();
  if (!options.exampleOnly) {
    const configPath = (0, import_node_path7.resolve)(rootDir, "env-doctor.config.js");
    if (await fileExists(configPath) && !options.force) {
      logger.warn("Config file already exists. Use --force to overwrite.");
    } else {
      await (0, import_promises4.writeFile)(configPath, generateConfigTemplate());
      logger.success(`Created ${import_picocolors3.default.cyan("env-doctor.config.js")}`);
    }
  }
  if (!options.configOnly) {
    const examplePath = (0, import_node_path7.resolve)(rootDir, ".env.example");
    if (await fileExists(examplePath) && !options.force) {
      ...
      await (0, import_promises4.writeFile)(examplePath, templateContent);
      logger.success(`Created ${import_picocolors3.default.cyan(".env.example")} with ${uniqueVars.length} variables`);
    }
  }
  console.log();
  console.log(import_picocolors3.default.gray("Next steps:"));
  console.log(import_picocolors3.default.gray("  1. Edit env-doctor.config.js to customize rules"));
  console.log(import_picocolors3.default.gray("  2. Run `env-doctor` to scan your project"));
  console.log();
}
async function runFix(options) {
  const rootDir = process.cwd();
  console.log();
  console.log(import_picocolors3.default.bold(import_picocolors3.default.cyan("env-doctor")) + " fix");
  console.log();
  try {
...
Found ${missingIssues.length} missing variable(s)`));
      for (const issue of missingIssues) {
        ...
        if (action === "add") {
          ...
          if (!options.dryRun) {
            const envPath = (0, import_node_path7.resolve)(rootDir, ".env");
            const content = `
${issue.variable}=${value}
`;
            await (0, import_promises4.writeFile)(envPath, content, { flag: "a" });
...
Found ${syncIssues.length} sync issue(s)`));
    ...
    if (recommendations.length > 0) {
      console.log();
      console.log(import_picocolors3.default.bold("Security Recommendations:"));
      for (const rec of recommendations) {
        console.log(import_picocolors3.default.gray(`  \u2022 ${rec}`));
      }
    }
    console.log();
  } catch (err) {
...
async function runScanHistory(options) {
  ...
  console.log();
  console.log(import_picocolors3.default.bold(import_picocolors3.default.cyan("env-doctor")) + " scan-history");
  console.log();
...
`));
  for (const result of results) {
    console.log(import_picocolors3.default.bold(`  ${result.variable}`));
    console.log(import_picocolors3.default.gray(`    Commit: ${result.commit.slice(0, 8)}`));
    console.log(import_picocolors3.default.gray(`    File: ${result.file}:${result.line}`));
    console.log(import_picocolors3.default.gray(`    Author: ${result.author}`));
    console.log(import_picocolors3.default.gray(`    Date: ${result.date}`));
    console.log(import_picocolors3.default.gray(`    Value: ${result.redactedValue}`));
    console.log();
  }
  console.log(import_picocolors3.default.yellow("Recommendation: Consider rotating these credentials and using git-filter-branch or BFG to remove them from history."));
  console.log();
  process.exit(1);
}
async function runWatch(options) {
  const rootDir = process.cwd();
  console.log();
  console.log(import_picocolors3.default.bold(import_picocolors3.default.cyan("env-doctor")) + " watch");
  console.log(import_picocolors3.default.gray("Watching for changes... Press Ctrl+C to stop."));
  console.log();
  const { config } = await loadConfig(options.config, rootDir);
  ...
  const runAnalysis = async () => {
    try {
      ...
      if (!analysisResult || result.issues.length !== analysisResult.issues.length || result.stats.errorCount !== analysisResult.stats.errorCount) {
        console.clear();
        console.log(import_picocolors3.default.bold(import_picocolors3.default.cyan("env-doctor")) + " watch");
        console.log(import_picocolors3.default.gray(`Last updated: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`));
        console.log();
  ...
  for (const envFile of config.envFiles) {
    const envPath = (0, import_node_path7.resolve)(rootDir, envFile);
    try {
      watch(envPath, () => {
  ...
  const srcDir = (0, import_node_path7.resolve)(rootDir, "src");
  try {
  ...
  process.on("SIGINT", () => {
    console.log();
    console.log(import_picocolors3.default.gray("Stopped watching."));
    process.exit(0);
  });
  await new Promise(() => {
  });
