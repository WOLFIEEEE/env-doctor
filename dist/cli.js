...
// src/types/config.ts
import { z } from "zod";
var VariableRuleSchema, EnvDoctorConfigSchema, defaultConfig;
var init_config = __esm({
  "src/types/config.ts"() {
    "use strict";
    init_esm_shims();
    VariableRuleSchema = z.object({
    ...
    EnvDoctorConfigSchema = z.object({
...
async function loadConfig(configPath, rootDir = process.cwd()) {
  if (configPath) {
    const absolutePath = resolve2(rootDir, configPath);
  ...
  for (const filename of CONFIG_FILENAMES) {
    ...
    if (foundPath) {
      ...
      return {
        config: mergeWithDefaults(config, dirname2(foundPath)),
        configPath: foundPath
      };
    }
  }
  const packageJsonPath = resolve2(rootDir, "package.json");
...
async function parseEnvFile(filePath, rootDir = process.cwd()) {
  const absolutePath = resolve3(rootDir, filePath);
  const variables = [];
  const errors = [];
  if (!await fileExists(absolutePath)) {
...
async function scanCode(options) {
  ...
  for (const file of files) {
    try {
      ...
      const message = err instanceof Error ? err.message : "Unknown error";
      errors.push({ file: relative2(rootDir, file), message });
...
async function detectFramework(rootDir) {
  ...
  for (const [framework, info] of Object.entries(FRAMEWORKS)) {
    for (const configFile of info.configFiles) {
      const configPath = resolve5(rootDir, configFile);
  ...
  const packageJsonPath = resolve5(rootDir, "package.json");
...
function reportToConsole(result, options = {}) {
  ...
  console.log();
  console.log(pc2.bold(pc2.cyan("env-doctor")) + pc2.gray(" v1.0.0"));
  console.log();
  const issuesByType = groupIssuesByType(result.issues);
  console.log(pc2.gray(`Framework: ${result.framework}`));
  console.log(
    pc2.gray(
  ...
  for (const [type, issues] of Object.entries(issuesByType)) {
    ...
    console.log(color(pc2.bold(`${icon} ${label} (${issues.length} ${issues.length === 1 ? "issue" : "issues"})`)));
    console.log();
    ...
    if (issues.length > maxIssuesPerCategory) {
      console.log(
        pc2.gray(`  ... and ${issues.length - maxIssuesPerCategory} more`)
      );
    }
    console.log();
  }
  if (!issuesByType["sync-drift"]?.length && result.templateVariables) {
    console.log(pc2.green("\u2713 ") + pc2.bold("Sync Check"));
    console.log(pc2.gray("  .env and template are in sync"));
    console.log();
  }
  printSummary(result);
  if (result.stats.errorCount > 0) {
    console.log();
    console.log(pc2.bold("Recommendations:"));
    console.log(pc2.gray("  Run `env-doctor fix` to interactively resolve issues"));
    console.log(pc2.gray("  Run `env-doctor init` to generate a .env.example file"));
  }
  if (!hasIssues) {
...
function printIssue(issue, verbose) {
  ...
  console.log(`  ${pc2.bold(variable)}`);
  console.log(`    ${pc2.gray(message)}`);
  if (location) {
    const loc = location.column ? `${location.file}:${location.line}:${location.column}` : `${location.file}:${location.line}`;
    console.log(`    ${pc2.dim("at")} ${pc2.cyan(loc)}`);
  }
  if (fix && verbose) {
    console.log(`    ${pc2.green("fix:")} ${fix}`);
  }
  console.log();
}
function printSummary(result) {
  const { errorCount, warningCount, infoCount, duration } = result.stats;
  console.log(pc2.gray("\u2500".repeat(50)));
  console.log();
  const parts = [];
  if (errorCount > 0) {
    parts.push(pc2.red(`${errorCount} ${errorCount === 1 ? "error" : "errors"}`));
  }
  if (warningCount > 0) {
    parts.push(pc2.yellow(`${warningCount} ${warningCount === 1 ? "warning" : "warnings"}`));
  }
  if (infoCount > 0) {
    parts.push(pc2.blue(`${infoCount} info`));
  }
  if (parts.length > 0) {
    console.log(`Summary: ${parts.join(", ")}`);
  } else {
    console.log(pc2.green("Summary: All checks passed!"));
  }
  console.log(pc2.gray(`Completed in ${duration}ms`));
  console.log();
}
...
function createSpinner(text) {
  ...
  return {
    start() {
      if (isTTY) {
        process.stdout.write(`${pc2.cyan(frames[frameIndex])} ${currentText}`);
        ...
        console.log(`${pc2.cyan("...")} ${currentText}`);
      }
    },
    stop(success = true) {
      if (intervalId) {
    ...
    for (const commit of commits) {
      ...
      for (const secret of secrets) {
        results.push({
          ...
          redactedValue: redactValue2(secret.value)
        });
      }
    }
...
function isLikelySecret(variable, value) {
  ...
  for (const pattern of SECRET_VALUE_PATTERNS2) {
    if (pattern.test(value)) {
      return true;
    }
  }
...
program.name("env-doctor").description("Analyze and validate environment variables in your codebase").version(VERSION);
program.argument("[directory]", "Directory to scan", process.cwd()).option("-c, --config <path>", "Path to config file").option("-e, --env <environment>", "Target environment (development, production, test)").option("-f, --format <format>", "Output format (console, json, sarif)", "console").option("--ci", "CI mode - exit with code 1 on errors").option("-v, --verbose", "Verbose output").option("--strict", "Treat warnings as errors").action(async (directory, options) => {
  await runScan(directory, options);
});
program.command("init").description("Initialize env-doctor in your project").option("--config-only", "Only create config file").option("--example-only", "Only create .env.example").option("-f, --force", "Overwrite existing files").action(async (options) => {
  await runInit(options);
});
program.command("fix").description("Interactively fix environment issues").option("-c, --config <path>", "Path to config file").option("--dry-run", "Show what would be fixed without making changes").action(async (options) => {
  await runFix(options);
});
program.command("scan-history").description("Scan git history for leaked secrets").option("-d, --depth <number>", "Number of commits to scan", "100").option("-f, --format <format>", "Output format (console, json)", "console").action(async (options) => {
  await runScanHistory(options);
});
program.command("watch").description("Watch for changes and re-analyze").option("-c, --config <path>", "Path to config file").action(async (options) => {
  await runWatch(options);
});
program.parse();
async function runScan(directory, options) {
  const rootDir = resolve6(directory);
  if (options.verbose) {
    logger.setVerbose(true);
  }
  try {
    ...
    const hasErrors = config.strict ? result.stats.errorCount > 0 || result.stats.warningCount > 0 : result.stats.errorCount > 0;
...
async function runInit(options) {
  ...
  console.log();
  console.log(pc3.bold(pc3.cyan("env-doctor")) + " init");
  console.log();
  if (!options.exampleOnly) {
    const configPath = resolve6(rootDir, "env-doctor.config.js");
    if (await fileExists(configPath) && !options.force) {
      ...
      logger.success(`Created ${pc3.cyan("env-doctor.config.js")}`);
    }
  }
  if (!options.configOnly) {
    const examplePath = resolve6(rootDir, ".env.example");
    if (await fileExists(examplePath) && !options.force) {
      logger.warn(".env.example already exists. Use --force to overwrite.");
    } else {
      const { config } = await loadConfig(void 0, rootDir);
      const result = await analyze({ config });
      const allVars = [
        ...result.definedVariables,
        ...result.usedVariables.filter((u) => u.name !== "<dynamic>").filter((u) => !result.definedVariables.find((d) => d.name === u.name)).map((u) => ({
          name: u.name,
          value: "",
          line: 0,
          file: ".env.example"
      ...
      logger.success(`Created ${pc3.cyan(".env.example")} with ${uniqueVars.length} variables`);
    }
  }
  console.log();
  console.log(pc3.gray("Next steps:"));
  console.log(pc3.gray("  1. Edit env-doctor.config.js to customize rules"));
  console.log(pc3.gray("  2. Run `env-doctor` to scan your project"));
  console.log();
}
async function runFix(options) {
  const rootDir = process.cwd();
  console.log();
  console.log(pc3.bold(pc3.cyan("env-doctor")) + " fix");
  console.log();
  try {
...
Found ${missingIssues.length} missing variable(s)`));
      for (const issue of missingIssues) {
        ...
        if (action === "add") {
          ...
          if (!options.dryRun) {
            const envPath = resolve6(rootDir, ".env");
            const content = `
...
Found ${syncIssues.length} sync issue(s)`));
    ...
    if (recommendations.length > 0) {
      console.log();
      console.log(pc3.bold("Security Recommendations:"));
      for (const rec of recommendations) {
        console.log(pc3.gray(`  \u2022 ${rec}`));
      }
    }
    console.log();
  } catch (err) {
...
async function runScanHistory(options) {
  ...
  console.log();
  console.log(pc3.bold(pc3.cyan("env-doctor")) + " scan-history");
  console.log();
...
`));
  for (const result of results) {
    console.log(pc3.bold(`  ${result.variable}`));
    console.log(pc3.gray(`    Commit: ${result.commit.slice(0, 8)}`));
    console.log(pc3.gray(`    File: ${result.file}:${result.line}`));
    console.log(pc3.gray(`    Author: ${result.author}`));
    console.log(pc3.gray(`    Date: ${result.date}`));
    console.log(pc3.gray(`    Value: ${result.redactedValue}`));
    console.log();
  }
  console.log(pc3.yellow("Recommendation: Consider rotating these credentials and using git-filter-branch or BFG to remove them from history."));
  console.log();
  process.exit(1);
}
async function runWatch(options) {
  const rootDir = process.cwd();
  console.log();
  console.log(pc3.bold(pc3.cyan("env-doctor")) + " watch");
  console.log(pc3.gray("Watching for changes... Press Ctrl+C to stop."));
  console.log();
  const { config } = await loadConfig(options.config, rootDir);
  ...
  const runAnalysis = async () => {
    try {
      const result = await analyze({ config });
      if (!analysisResult || result.issues.length !== analysisResult.issues.length || result.stats.errorCount !== analysisResult.stats.errorCount) {
        console.clear();
        console.log(pc3.bold(pc3.cyan("env-doctor")) + " watch");
        console.log(pc3.gray(`Last updated: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`));
        console.log();
  ...
  for (const envFile of config.envFiles) {
    const envPath = resolve6(rootDir, envFile);
    try {
      watch(envPath, () => {
  ...
  const srcDir = resolve6(rootDir, "src");
  try {
  ...
  process.on("SIGINT", () => {
    console.log();
    console.log(pc3.gray("Stopped watching."));
    process.exit(0);
  });
  await new Promise(() => {
  });
